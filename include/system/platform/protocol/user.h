...
void UpdatePosition()
{
	...
	hipAngle = hipBaseSym*(hipAngle - hipBaseOff);     // 减去零度角基准
	hipAngle = hipAngle*3.1415/180;
	
	yawAngleoff  = getJointCoder(yawCoder);
	yawAngle  =  36*yawAngleoff/4096*10;      //100倍放大
	yawAngle = yawBaseSym*(yawAngle - yawBaseOff);    // 减去零度角基准
	yawAngle = yawAngle*3.1415/180;
	
	pressValue = getJointCoder(pressSensor);			//获取足底压力传感器压力值
